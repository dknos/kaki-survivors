...
export function buildEnv(scene, renderer) {
  ...
  // P4A cohort 3 — dedicated cave wet-stone pack. Replaces the brown_mud
  // fallthrough used in cohort 2. Procedural PNGs generated by
  // tools/_gen_cave_stone_texture.mjs (mulberry32 seeded → byte-identical
  // re-runs). PNG paths instead of jpg keep the alpha-free 24-bit RGB the
  // generator emits without lossy compression. The diffuse is pure
  // luminance — slot-2 wet-stone hue comes from STAGES.cave.groundTint
  // (0x4a4a52) multiplied into ground.material.color, keeping the cave
  // pack palette-locked per docs/CAVE_VISUAL_STYLE.md.
  function loadPngTex(url, srgb) {
    const t = loader.load(url, tex => tex.needsUpdate = true);
    t.wrapS = t.wrapT = THREE.RepeatWrapping;
    t.repeat.set(repeat, repeat);
    t.anisotropy = Math.min(maxAniso, 8);
    if (srgb) t.colorSpace = THREE.SRGBColorSpace;
  ...
  groundPacks.cave = {
    diff:   loadPngTex('assets/textures/cave_stone_diffuse.png', true),
    rough:  loadPngTex('assets/textures/cave_stone_rough.png',   false),
    normal: loadPngTex('assets/textures/cave_stone_normal.png',  false),
  };
  ...
  group.userData.applyStageTint = (stage) => {
    ...
    // cave gets a dedicated wet-stone pack (P4A cohort 3 — drops the brown_mud
    // fallthrough used in cohort 2).
    const packKey = isCave ? 'cave' : (isForest ? 'forest' : 'twilight');
